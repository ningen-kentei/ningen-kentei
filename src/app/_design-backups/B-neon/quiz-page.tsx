...
export default function QuizPage() {
  ...
  return (
    <div className="min-h-screen bg-mesh text-white flex flex-col">
      {/* Progress bar */}
      <div className="sticky top-0 z-10 bg-[#0a0a12]/95 backdrop-blur-sm px-4 py-3 border-b border-white/5">
        ...
        <div className="w-full h-1.5 bg-gray-800/80 rounded-full overflow-hidden">
          <div
            className="h-full rounded-full gradient-bar transition-all duration-500 ease-out"
          ...
          {question.shuffledChoices.map((choice, i) => {
            ...
            return (
              <button
                ...
                className={`group w-full text-left px-5 py-4 rounded-xl transition-all duration-200 border ${
                  isSelected
                    ? "border-purple-500 bg-purple-500/15 shadow-lg shadow-purple-500/10"
                    : "border-white/10 bg-white/[0.03] hover:bg-white/[0.07] hover:border-purple-500/40"
                }`}
              >
                <span
                  className={`inline-block w-7 h-7 mr-3 text-center leading-7 text-xs font-black rounded-full transition-colors ${
                    isSelected
                      ? "bg-gradient-to-r from-purple-500 to-pink-500 text-white"
                      : "bg-white/10 text-gray-400 group-hover:text-purple-300"
                  }`}
                >
                  {labels[i]}
                </span>
