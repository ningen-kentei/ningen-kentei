...
import type { TekishokuChoice } from "@/lib/tekishoku/types";
...
type ShuffledChoice = TekishokuChoice & { originalIndex: number };

const labels = ["A", "B", "C", "D", "E"];

export default function TekishokuQuizPage() {
  const router = useRouter();
  const [shuffledQuestions] = useState(() =>
    shuffleArray(tekishokuQuestions).map((q) => {
  ...
  const total = tekishokuQuestions.length;

  useEffect(() => {
    router.prefetch("/tekishoku/result");
  }, [router]);
  ...
  const handleSelect = useCallback(
    (shuffledIdx: number) => {
      ...
      const originalIndex =
        question.shuffledChoices[shuffledIdx].originalIndex;
      ...
      setTimeout(
        () => {
          ...
          if (isLast) {
            ...
            setTimeout(() => {
              router.push(`/tekishoku/result?q=${qIds}&a=${encoded}`);
            }, 1500);
            return;
          }
    ...
    return (
      <div className="min-h-screen bg-dotgrid text-white flex flex-col items-center justify-center">
        <div className="text-center">
          <div className="inline-block border-4 border-emerald-400 px-6 py-2 mb-6">
            <span className="text-emerald-400 text-sm font-black tracking-[0.3em]">
              ANALYZING
            </span>
          </div>
          <p className="text-2xl font-black mb-4 animate-pulse">分析中...</p>
          <p className="text-sm text-gray-500">
            あなたの適職タイプを分析しています
          </p>
          <div className="mt-8 w-48 h-1 bg-gray-800 mx-auto overflow-hidden">
            <div className="h-full bg-emerald-400 animate-[loading_1.5s_ease-in-out_infinite]" />
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-dotgrid text-white flex flex-col">
      ...
      <div className="sticky top-0 z-10 bg-[#0e0e1a]/95 backdrop-blur-sm px-4 py-3 border-b border-white/5">
        ...
        <div className="w-full h-1 bg-gray-800/80 overflow-hidden">
          <div
            className="h-full transition-all duration-500 ease-out bg-emerald-400"
      ...
      >
        ...
        <div className="w-full space-y-3">
          {question.shuffledChoices.map(
            (choice: ShuffledChoice, i: number) => {
                    ...
                    className={`group w-full text-left px-5 py-4 transition-all duration-200 border-l-4 flex items-center gap-3 ${
                      isSelected
                        ...
                        : isNone
                          ...
                          : "border-l-gray-700 bg-white/[0.05] hover:bg-white/[0.09] hover:border-l-emerald-400/60"
                    }`}
                  >
                    <span
                      className={`shrink-0 w-7 h-7 text-center leading-7 text-xs font-black transition-colors ${
                        isSelected
                          ...
                          : isNone
                            ...
                            : "bg-gray-700 text-gray-300 group-hover:text-emerald-400"
                      }`}
                    >
                      {isNone ? "-" : labels[i]}
                    </span>
                    <span className={`text-sm sm:text-base ${isNone ? "text-gray-400" : ""}`}>
                      {choice.text}
                    </span>
                  </button>
                </div>
